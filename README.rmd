...
pacman::p_load(tidyverse, foreign, haven, estimatr, broom, kableExtra, FactoMineR, pcaMethods, missMDA)
...
#   
#   beta <- theta[1:ncol(X)]
#   sigma <- exp(theta[ncol(X) + 1])  # ensures sigma > 0
#   
#   mu <- X %*% beta
#   
...
# }
# 
# # prepare matrix of relevant variables
# 
# y <- ols_df$l_total_inc
# x <- cbind(1, ols_df$w4_best_edu, ols_df$w1_a_consc)
# 
# x2 <- as.data.frame(x) %>% 
#     filter(!is.na(V2)) %>% 
#     filter(!is.na(V3))
